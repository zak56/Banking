			...
			<tr>
				<th>
					<div id="HolidaysUserInput">
						<%
							...
							if (request.getParameter("submit") != null) {
								...
								try {
									...
									String sql = "INSERT INTO `location service` (`ServiceID`, `LocationID`, `Service`) VALUES (?,?,?);";
									pstatement = connection.prepareStatement(sql);
									pstatement.setString(1, ServiceID);
						...
						<form method="get">
							<fieldset>
								<label>Service ID: <input type="text" name="ServiceID" /><br></label>
								<label>Location ID: <input type="text" name="LocationID" /><br></label>
								<label>Service Name: <input type="text" name="Service" /><br></label>
							</fieldset>
							<br> <input type="submit" name="submit"
								value="Add New Service" /><br>
							<br>
						</form>
					</div>
					...
					<div>
						<%
							...
							if (request.getParameter("removeSubmit") != null) {
								...
								try {
									...
									String sql = "DELETE FROM `location service` WHERE `location service`.`ServiceID` = ?";
									pstatement = connection.prepareStatement(sql);
						...
						<form method="get">
							<fieldset>
								<label>Service ID: <input type="text" name="RemoveID" /><br></label>
							</fieldset>
							<br> <input type="submit" name="removeSubmit"
								value="Remove a Service from Location" />
						</form>
										...
										<table border="1">
											...
											<%
												try {
													connection = DriverManager.getConnection(connectionUrl + database, userid, password);
													statement = connection.createStatement();
													String sql = "select * from `location service` ORDER BY `location service`.`ServiceID` ASC";
